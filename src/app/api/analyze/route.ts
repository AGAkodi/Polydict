...
export async function POST(request: Request) {
  try {
    const { market, signals } = await request.json();
    ...
    if (!apiKey) {
      console.warn('[Simulator] ANTHROPIC_API_KEY env variable is missing. Running analyze in simulator mode.');
    ...
    const currentYesOdds = market.yesPrice; // 0.0 - 1.0 representation
...
Analyze this data and return the structured JSON.`;
    ...
    try {
      ...
      console.error('Failed to parse Claude output directly, trying clean parse:', contentText);
      try {
        parsedAnalysis = parseCleanJson(contentText.substring(contentText.indexOf('{'), contentText.lastIndexOf('}') + 1));
      } catch (nestedErr) {
        throw new Error('Claude response did not contain parseable JSON: ' + contentText);
      }
    }
